...
var token = new JwtSecurityToken(
    issuer: "Registry",
    audience: "TheRegistry",
    claims: [new Claim("role", "agent")],
...
configuration.Bind("ApplicationOptions", applicationOptions);
...
{
    options.Endpoint = new(agent.Url, "/a2a");
...
var menu = new Grid()
    ...
    .AddRow("[bold yellow]list all agents[/]", string.Empty, "Display all registered agents")
        ...
        if (applicationOptions.Streaming is true)
        {
            ...
            try
            {
                await foreach (var response in client.SendTaskStreamingAsync(request, agentCts.Token))
                {
                    ...
                    {
                        var msg = evt.Status.Message?.Parts?.FirstOrDefault()?.ToText() ?? string.Empty;
...
void printCard(AgentCard card)
{
    ...
    {
        var credentialsText = !string.IsNullOrWhiteSpace(card.Authentication.Credentials)
    ? new Text(JsonSerializer.Serialize(card.Authentication.Credentials), new Style(foreground: Color.Green))
    : new Text("None");

        var authDetailsTable = new Table()
            .AddColumn("[underline]Schemes[/]", c => c.NoWrap().Centered())
            .AddColumn(new TableColumn(new Markup("[underline]Credentials[/]").Centered()))
            .NoBorder()
            .RoundedBorder()
            .AddRow(
                new Text(card.Authentication?.Schemes?.Count is null or 0
                    ? "None"
